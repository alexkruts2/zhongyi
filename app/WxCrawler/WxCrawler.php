...
class WxCrawler
{
	...
	public function __construct($host='', $referer='', $proxy=false)
	{
		...
		/** @var 处理微信图片的防盗链 */
		$this->antiLeech = 'http://'.$_SERVER['SERVER_NAME'].'/admin/getWechatImage?url=';
	}
	/**
	 * 爬取内容
	 * @author bignerd
...
//		curl_setopt_array($ch,$options);
	...
	public function contentHandle($content)
	{
        ...
        $content_html = preg_replace_callback('/data-src="(.*?)"/', function($matches){
            ...
            $imageObj = download_image($url);

            $checkImage = file_exist_in_cos($imageObj['filename'],'image');
            if (is_null($checkImage)){
                $tengxun_result = save_to_cos($imageObj['filename'], 'image');
                $pic = Image::firstOrCreate(
                    ['md5' => $imageObj['md5']],
                    [
                        'base_url' => config('asset.cos_img_url'),
            ...
            delete_file(config('asset.image_path'), $imageObj['filename']);


            return 'src=' . $pic->base_url . '/' . $pic->filename;
